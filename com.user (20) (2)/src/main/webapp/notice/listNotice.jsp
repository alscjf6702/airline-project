...
<ul class="pagination pagination justify-content-center">
   <c:choose>
         <c:when test="${pageVO.pageNum >= (pageVO.total/10)+1 || pageVO.pageNum==1}">
      <li class="page-item" ><a class="page-link" onclick="return previous()">Previous</a></li>
      </c:when>
         <c:otherwise>
      <li class="page-item"><a class="page-link" href="BoardServlet?command=notice_list&pageNum=${pageVO.pageNum - 1 }&amount=${pageVO.amount}">Previous</a></li>
                   </c:otherwise>
                </c:choose>
              <c:forEach var="num" begin="${pageVO.startPage }"
                ...
                <c:choose>
                    <c:when test="${((pageVO.total/10)) <= pageVO.pageNum}">
                       <li class="page-item"><a class="page-link" onclick="return next()">Next</a></li>
                   </c:when>
                       <c:otherwise>
                          <li class="page-item"><a class="page-link" href="BoardServlet?command=notice_list&pageNum=${pageVO.pageNum + 1 }&amount=${pageVO.amount}">Next</a></li>
