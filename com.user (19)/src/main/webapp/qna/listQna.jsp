...
<body>
	...
	<form action="BoardServlet" method="post" name="frm">
		<input type="hidden" name="command" value="search_qna"> <span>
			▷ 총 ${pageVO.total }개의 게시물이 있습니다. </span> <span class="right"> 
...
<ul class="pagination pagination justify-content-center">
	<c:choose>
		...
		<li class="page-item"><a class="page-link" onclick="return previous()">Previous</a></li>
   		...
   		<c:otherwise>
		<li class="page-item"><a class="page-link" href="BoardServlet?command=qna_list&pageNum=${pageVO.startPage - 1 }&amount=${pageVO.amount}">Previous</a></li>
					    </c:otherwise>
				    </c:choose>
				  <c:forEach var="num" begin="${pageVO.startPage }"
				end="${pageVO.endPage }">
				    	<li class="page-item" style="color:black;"><a class="page-link" href="BoardServlet?command=qna_list&pageNum=${num }&amount=${pageVO.amount}">${num }</a></li>
				    </c:forEach>
 					<c:choose>
					  	<c:when test="${((pageVO.total/10)+1) < pageVO.pageNum}">
   					  	...
   					  	<c:otherwise>
   					  		<li class="page-item"><a class="page-link" href="BoardServlet?command=qna_list&pageNum=${pageVO.endPage + 1 }&amount=${pageVO.amount}">Next</a></li>
	...
	<script>
		function change(a) {
			//console.log(a);
			//console.log(a.value);
			location.href = "BoardServlet?command=qna_list&pageNum=1&amount="
					+ a.value;
		}
	</script>
