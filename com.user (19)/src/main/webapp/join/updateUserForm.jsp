	...
	<div class="tm-main-content" id="top">
		<div class="tm-page-wrap mx-auto">
			...
			<form action="UserServlet" name="frm" method="post">
				...
				<table class="table_form" id="table" border="1" style="margin: auto; border-collapse: collapse;">
					<tbody>

						<tr>
							<th scope="row"><label for="korName">한글명<span
									class="icon_require"><br>필수항목</span></label></th>
							<td><input type="text" name="korName" value="${loginUser.korName}"> 
								<p class="txt_error_Msg" id="error_koreanName"
									style="display: none;"></p>
							</td>
						</tr>
						<tr>
							<th scope="row">영문명<span class="icon_require"><br>필수항목</span></th>
							<td><input type="text" name="engName" value="${loginUser.engName}"> </td>
						</tr>

						<tr>
							<th scope="row">성별<span class="icon_require"><br>필수항목</span></th>
							<td><c:set var="gender" value="${loginUser.gender}" /> 
							<c:if test="${gender%2 == 1}">M</c:if> 
							<c:if test="${gender%2 == 0}">F</c:if></td>
						</tr>

						<tr>
							<th scope="row">생년월일<span class="icon_require"></span></th>
						...
						<tr>
							<th scope="row"><label for="input_pw">비밀번호<span
									class="icon_require"></span></label></th>
							<td><input type="password" id="pwd" autocomplete="off"
								name="pwd" placeholder="영문+숫자+특수문자 6~10자리" maxlength="10"
								title="영문+숫자+특수문자 6~10자리" style="width: 298px;"
								onkeydown="inputPwdChk()">
						...
						<tr>
							<th scope="row"><label for="pwd_Check">비밀번호 확인<span
									class="icon_require"></span></label></th>
							<td><input type="password" id="pwd_Check"
								autocomplete="off" placeholder="영문+숫자+특수문자 6~10자리" title="영문+숫자+특수문자 6~10자리"
								maxlength="10" style="width: 298px;" onkeydown="inputPwdChk()">

								<button onclick="joinInformPwdCheck()" id="btn_pwdCheck"
									type="button" class="btn_S white">확인</button> <input
								type="hidden" name="pwdDuplication" value="pwdUncheck" />
							</td>
						</tr>
						<tr>
							<th scope="row"><label>휴대전화<span
									class="icon_require">필수항목</span></label></th>
							<td id="korea_phone">
							<select id="phone_first" name="phone_first"
								style="width: 120px" title="휴대전화 번호 앞자리">
									
								<option value="">선택</option>
								<option value="010" <c:if test="${fn:substring(loginUser.phone,0,3)==010}">selected="selected"</c:if>>010</option>
								<option value="011" <c:if test="${fn:substring(loginUser.phone,0,3)==011}">selected="selected"</c:if>>011</option>
								<option value="016" <c:if test="${fn:substring(loginUser.phone,0,3)==016}">selected="selected"</c:if>>016</option>
								<option value="017" <c:if test="${fn:substring(loginUser.phone,0,3)==017}">selected="selected"</c:if>>017</option>
								<option value="018" <c:if test="${fn:substring(loginUser.phone,0,3)==018}">selected="selected"</c:if>>018</option>
								<option value="019" <c:if test="${fn:substring(loginUser.phone,0,3)==019}">selected="selected"</c:if>>019</option>
							</select> 
							<input type="text" id="phone_middle" name="phone_middle"
								placeholder="중간번호" title="휴대전화 번호 가운데 자리" maxlength="4"
								style="width: 150px;" value="${fn:substring(loginUser.phone,4,8)}" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1')">
							<input type="text" id="phone_last" name="phone_last" 
								placeholder="끝 번호" title="휴대전화번호 끝자리"
								maxlength="4" style="width: 150px;" value="${fn:substring(loginUser.phone,9,13)}" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1')">
						...
						<tr>
							<th scope="row"><label for="input_emailID">이메일<span
									class="icon_require">필수항목</span></label></th>
							<td><input type="text" id="input_emailID" name="email"
								placeholder="이메일 입력" title="이메일 아이디 입력" style="width: 198px;" value="${fn:split(loginUser.mail,'@')[0]}">
								@ &nbsp; 
								<select id="select_emailDomain" title="이메일 도메인 선택" name="mail_Domain"
								style="width: 180px;">
									<option value="" id="option_directInput"
									onclick="return display()">직접입력</option>
									
									<option value="naver.com" onclick="display()" <c:if test="${fn:split(loginUser.mail,'@')[1]=='naver.com'}">selected="selected"</c:if>>naver.com</option>

									<option value="yahoo.co.kr" onclick="display()" <c:if test="${fn:split(loginUser.mail,'@')[1]=='yahoo.co.kr'}">selected="selected"</c:if>>yahoo.co.kr</option>

									<option value="hotmail.com" onclick="display()" <c:if test="${fn:split(loginUser.mail,'@')[1]=='hotmail.com<'}">selected="selected"</c:if>>hotmail.com</option>

									<option value="hanmail.net" onclick="display()" <c:if test="${fn:split(loginUser.mail,'@')[1]=='hanmail.net'}">selected="selected"</c:if>>hanmail.net</option>

									<option value="nate.com" onclick="display()" <c:if test="${fn:split(loginUser.mail,'@')[1]=='nate.com'}">selected="selected"</c:if>>nate.com</option>

									<option value="yahoo.com" onclick="display()" <c:if test="${fn:split(loginUser.mail,'@')[1]=='yahoo.com'}">selected="selected"</c:if>>yahoo.com</option>

									<option value="gmail.com" onclick="display()" <c:if test="${fn:split(loginUser.mail,'@')[1]=='gmail.com'}">selected="selected"</c:if>>gmail.com</option>

									<option value="hotmail.co.kr" onclick="display()" <c:if test="${fn:split(loginUser.mail,'@')[1]=='hotmail.co.kr'}">selected="selected"</c:if>>hotmail.co.kr</option>

							</select> <input type="text" id="input_emailDomain" placeholder="직접입력"
								title="직접입력" style="width: 298px; display: inline-block;" value="${fn:split(loginUser.mail,'@')[1]}" disabled="disabled"></td>
						</tr>

						<tr>
							<th scope="row"><label for="input_defaultAddress">주소<br></label></th>
							<td id="korea_address">
							<input type="text"
								id="sample6_postcode" name="postcode" placeholder="우편번호" value="${loginUser.postcode}"> 
							<input type="button" onclick="sample6_execDaumPostcode()"
								value="우편번호 찾기"><br> 
							<input type="text" id="sample6_address" name="address" placeholder="주소" value="${loginUser.address}"><br> 
							...
							<!-- <td id="foreign_address" style="display: none;">
								<p>
									<input type="text" id="input_abroadDefaultAddress"
										placeholder="기본 주소" title="기본 주소" style="width: 400px;">
									<input type="text" id="input_abroadDetailAddress"
										placeholder="상세 주소" title="상세 주소" style="width: 400px;">
								</p>
								<p class="mar_to15">
									<input type="text" id="input_abroadCity" placeholder="도시(City)"
										title="도시(City)" style="width: 257px;"> <input
										type="text" id="input_abroadZipCode"
										placeholder="우편번호(Zip Code)" title="우편번호(Zip Code)"
										style="width: 257px;">
								</p>
								<ul class="list_type3 fsz_14 mar_to10">
									<li>제공해주신 주소는 이벤트 또는 우수회원 승급 서비스 제공을 위해 이용됩니다.</li>
								...
								<p class="txt_error_Msg" id="error_abroadAddress"
									style="display: none;"></p>
							</td> -->
