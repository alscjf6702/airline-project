									...
									<c:if test="${not empty loginUser}">
										...
										<c:choose>
											<c:when test="${not empty checkin}">
												<td><button type="submit" class="btn btn-primary" onclick="location.href='UserServlet?command=buyticket&userid=${loginUser.userid}&num=${list.num}&department=${list.depairport}&arrive=${list.arrairport}&checkin=${list.regidate}'">구매</button></td>
											</c:when>
											<c:otherwise>
												<td><button type="submit" class="btn btn-primary" onclick="location.href='UserServlet?command=buyticket&userid=${loginUser.userid}&num=${list.num}'">구매</button></td>
											</c:otherwise>	
										</c:choose>
									</c:if>	
								</tr>
