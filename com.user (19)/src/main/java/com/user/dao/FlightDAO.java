...
public class FlightDAO {
	...
	public List<FlightVO> getBuyPaging(int curPage, int page) {
		int start = (curPage-1)*page + 1;
		int end = curPage*page;
		String sql = "SELECT * FROM(SELECT f.*, ROW_NUMBER() over(ORDER BY f.REGIDATE  desc) AS row_num FROM flightres f)WHERE row_num BETWEEN ? AND ?";
		List<FlightVO> list = new ArrayList<FlightVO>();
		FlightVO vo = null;
		Connection con = null;
		PreparedStatement ps = null;
	...
	public List<FlightVO> getBuySearchPaging(int curPage, int page, String userid, String formattedDate) {
		int start = (curPage-1)*page + 1;
		int end = curPage*page;
		String sql = "SELECT * FROM(SELECT f.*, ROW_NUMBER() over(ORDER BY f.REGIDATE  desc) AS row_num FROM flightres f where userid = ? and regidate = ?)WHERE row_num BETWEEN ? AND ?";
		List<FlightVO> list = new ArrayList<FlightVO>();
		FlightVO vo = null;
		Connection con = null;
		PreparedStatement ps = null;
